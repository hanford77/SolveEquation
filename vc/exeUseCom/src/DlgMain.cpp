...
int SolveEquation(const double z[10],double x[12])
{
    int(*pfnSolve[])(const double z[10],double x[12])   =
    {
        &Solve1MFC,
        &Solve2import,
        &Solve3MIDL_cpp,
    ...
    int nCount  =   sizeof(pfnSolve) / sizeof(pfnSolve[0]);
    return (*pfnSolve[rand() % nCount])(z,x);
}
